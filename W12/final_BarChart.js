class BarChart {
    ...
    init() {
        ...
        self.xaxis = d3.axisBottom(self.xscale)
            .tickValues(self.xscale.domain().filter(function(d,i){return !(i % 4);}))
            .tickSizeOuter(0);
    ...
    render() {
        ...
        self.chart.selectAll(".bar")
            ...
            .on('click', function(ev,d) {
                const is_active = filter.includes(d.weather);
                if ( is_active ) {
                    filter = filter.filter( f => f !== d.weather );
                }
                else {
                    filter.push( d.weather );
                }
                Filter();
